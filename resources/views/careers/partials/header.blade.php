...
<header class="ys-header" :class="scrolled ? 'ys-header--scrolled' : ''">
    <div class="ys-container ys-header__inner">
        <a href="{{ route('careers.index') }}" class="ys-brand" aria-label="صفحه فرصت‌هاي شغلي">
            <img src="{{ $logoUrl }}" alt="لوگوي {{ $companyName }}" class="ys-brand__logo" loading="eager" fetchpriority="high">
            <span class="ys-brand__text">فرصت‌هاي همکاري {{ $companyName }}</span>
        </a>

        <nav class="ys-nav" aria-label="ناوبري اصلي">
            <a href="{{ $headerAnchorBase }}#jobs">فرصت‌ها</a>
            <a href="{{ $headerAnchorBase }}#departments">تيم‌ها</a>
            <a href="{{ $headerAnchorBase }}#why-yadak">مزايا</a>
            <a href="{{ $headerAnchorBase }}#process">فرايند جذب</a>
            <a href="{{ $headerAnchorBase }}#faq">سوالات متداول</a>
        </nav>

        <div class="ys-header__actions">
            <button type="button" class="ys-icon-btn" @click="toggleTheme()" :aria-label="dark ? 'فعال‌سازي حالت روشن' : 'فعال‌سازي حالت تيره'">
            ...
            <button type="button" class="ys-menu-btn" @click="drawer = true" aria-label="باز کردن منو">
    ...
    >
        <div class="ys-drawer__head">
            <img src="{{ $logoUrl }}" alt="لوگوي {{ $companyName }}" class="h-10 w-auto">
            <button type="button" class="ys-icon-btn" @click="drawer = false" aria-label="بستن منو">×</button>
        </div>
        <nav class="ys-drawer__nav" aria-label="منوي موبايل">
            <a href="{{ $headerAnchorBase }}#jobs" x-ref="drawerFirstLink" @click="drawer = false">فرصت‌ها</a>
            <a href="{{ $headerAnchorBase }}#departments" @click="drawer = false">تيم‌ها</a>
            <a href="{{ $headerAnchorBase }}#why-yadak" @click="drawer = false">مزايا</a>
            <a href="{{ $headerAnchorBase }}#process" @click="drawer = false">فرايند جذب</a>
